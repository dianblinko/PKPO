...
class Handler(FileSystemEventHandler):
    ...
    def on_modified(self, event):
        ...
        for filename in os.listdir(folder_track):
            ...
            if len(extension) > 1 and (extension[1].lower() == "txt" or extension[1].lower() == "csv"):
                # то перемещаем файл в папку
                run_proc(folder_track + filename)
...
folder_track = 'D:/Учеба/3 курс/5 Семестр/ПКПО/flask_app/labapp/sourse_file/'
# Папка куда перемещать будем
folder_dest = 'D:/Учеба/3 курс/5 Семестр/ПКПО/flask_app/labapp/processed_file/'
