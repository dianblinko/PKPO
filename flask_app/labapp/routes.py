...
def index():
    ...
    return render_template('index.html', title='Статистика суицидов', pname='HOME', navmenu=controller.navmenu,
                           imgs=imgs)

@app.route('/aboutus')
def about_us():
    imgs = ['pha.jpg', 'phk.jpg', 'phv.jpg']
    # "рендеринг" (т.е. вставка динамически изменяемых данных) в index.html и возвращение готовой страницы
    return render_template('aboutus.html', title='О нас', pname='About us', navmenu=controller.navmenu,
                           imgs=imgs)

@app.route('/suicides_country')
def suicides_country():
    # Пример вызова метода с выборкой данных из БД и вставка полученных данных в html-шаблон
    processed_files = controller.get_suicides_country()
    # "рендеринг" (т.е. вставка динамически изменяемых данных) в index.html и возвращение готовой страницы
    return render_template('suicides_country.html', title='По странам', pname='Suicides country', navmenu=controller.navmenu, processed_files=processed_files)

@app.route('/suicides_year')
def suicides_year():
    # Пример вызова метода с выборкой данных из БД и вставка полученных данных в html-шаблон
    processed_files = controller.get_suicides_year()
    # "рендеринг" (т.е. вставка динамически изменяемых данных) в index.html и возвращение готовой страницы
    return render_template('suicides_year.html', title='По годам', pname='Suicides year', navmenu=controller.navmenu, processed_files=processed_files)

@app.route('/suicides_age')
def suicides_age():
    # Пример вызова метода с выборкой данных из БД и вставка полученных данных в html-шаблон
    processed_files = controller.get_suicides_age()
    # "рендеринг" (т.е. вставка динамически изменяемых данных) в index.html и возвращение готовой страницы
    return render_template('suicides_age.html', title='По возрасту', pname='Suicides age', navmenu=controller.navmenu, processed_files=processed_files)

# Обработка запроса к странице contact.html
@app.route('/contact')
def contact():
    return render_template('contact.html', title='Контакты', pname='CONTACT', navmenu=controller.navmenu)
